...
Courcelle's theorem in \cite{Courcelle2012}
\begin{thm}
...
%($2^{p(tw G)}$, $2^{2^{(\#Q)}}$,  
the constant factor can be very significant. An efficient implementation will still require several tricks to make this approach practical. To develop and debug these "tricks" might not always be that straight forward so that visualization can help at this point.\smallskip 
