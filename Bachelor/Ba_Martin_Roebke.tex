...
\lstset{tabsize=2, linewidth=1.05\textwidth, framextopmargin=5pt}
...
As two reference implementations of dynamic programming on tree decompositions we chose the existing solvers \href{https://github.com/daajoe/GPUSAT}{GPUSAT} and \href{https://github.com/hmarkus/dp_on_dbs}{dpdb}.
...
to utilize the graph structure of problems.
...
Python 3.8 because: Python 3.8 was at the beginning of the project the newest python version, released on October 14th 2019 (for a longer list see \href{https://docs.python.org/3/whatsnew/3.8.html}{summary of release highlights}). The change applied most times in this project would be \href{https://docs.python.org/3/whatsnew/3.8.html#f-strings-support-for-self-documenting-expressions-and-debugging}{f-string support} for shorter and easier to read string-building.
...
An example call with \textit{./gpusat -f ../examples/test\_da4\_1.cnf -v -p -d ../examples/td4p1.txt  -g ../examples/graphfileda41.txt -{}-visufile ../examples/visufileda41.json} enabled verbose output, disabled pre-processing to prevent the creation of bags with too many variables at once to be visualized, and creates
...
def create_json(problem: int, tw_file=None, intermed_nodes=False):
  ...
  with connect() as connection:
    ...
    if ptype == 'SharpSat':
        constructor = DpdbSharpSatVisu(connection, 
                                       problem, 
                                       intermed_nodes)
    elif ptype == 'VertexCover':
        constructor = DpdbMinVcVisu(connection, 
                                    problem, 
                                    intermed_nodes,                                     
                                    tw_file)
