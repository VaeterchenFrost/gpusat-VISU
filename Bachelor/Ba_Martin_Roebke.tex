...
We will start with a short introduction to MSO in \ref{sec:MSO}. Then we describe BFS as the basis for SAT and \#SAT problems, which are a popular application for solvers.
...
At the end of the chapter, we briefly describe the input and intermediate formats DIMACS and DOT.
...
As a short description of the new concepts we say that
...
A \textbf{relation symbol} with some \textbf{arity} (number of arguments) $\in \mathbb{N}$.
Relations of \textbf{arity 0} can be identified with propositional variables.
...
Function symbols of arity 0 are called constant symbols, and are often denoted by lowercase letters from the beginning of the alphabet a, b, c,... . \\

{First-order logic quantifies only variables that range over individuals (elements of the domain of discourse); \textbf{second-order logic}, in addition, also quantifies over relations.}
...
%\end{itemize}
Boolean satisfiability problem \textbf{SAT}: Given a propositional formula $\phi$, is there an assignment of the variables in $\phi$ for which $\phi$ is satisfied?

\textbf{\#SAT}: Given a propositional formula $\pi$, for how many assignments of the variables in $\pi$ is $\pi$ satisfied?
