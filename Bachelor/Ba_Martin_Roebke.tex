...
\begin{itemize}
	\item centerpad: float = 0, [image coordinates]
...
\begin{figure}[H]
	...
	\caption{Joining the blue (right side) image to the left gray image with only one parameter \textit{centerpadding} set to 200. We see the default vertical position v\_top=0, and the implied coordinate system with an origin in the top left corner.}
...
\begin{figure}[H]
	...
	\caption{Example f0r joining with no \textit{centerpadding}.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\linewidth]{images/svgjoinpad0.png}
	\caption{The second image gets scaled to the same size as the first image. This can be conveniently achieved by setting \textit{v\_top} to 0 and \textit{v\_bottom} to 1. Parameter \textit{centerpadding} is 200 as hinted.}
	\label{fig:svgjoinscale1}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\linewidth]{images/svgjoinpad0centered.png}
