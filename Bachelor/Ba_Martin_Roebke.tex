...
Now we take a look at one problem that is directly stated 
...
An instance $G=(V,E)$ of \textit{minimal vertex cover} parametrized by treewidth \textit{tw} can be solved in time $\mathcal{O}(2^{tw(G)}|V|)$ \cite{HenningParamAlgo}.
