...
%		https://de.wikipedia.org/wiki/Karps_21_NP-vollst%C3%A4ndige_Probleme#Liste_der_Probleme
		\item model-counting (\#P-problems) - {\normalfont\small Instead of \textbf{one} solution we want to count \textbf{all} solutions}
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Constructing and enriching the tree decomposition with solver information
		\item Adding the incidence graph for problems on Boolean formulas, from which primal and dual graphs can be derived
		\item Providing a discrete timeline over the solving process
		\item Parameters to control the quantity, coloring and, to a limited extent, the layout of the data
	\end{itemize}
\medskip
\textbf{Limitations:}
\begin{itemize}
